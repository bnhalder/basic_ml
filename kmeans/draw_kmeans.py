...
# -*- coding: utf-8 -*-
"""
Created on Tue Aug 22 19:25:20 2017
...
i=1
plot_clusters(x, y, "Vectors")
pylab.savefig(os.path.join(CHART_DIR, "1400_03_0%i.png"%i))
pylab.clf()

i += 1
mx, my = sp.meshgrid(sp.arange(0, 1, .0001), sp.arange(0, 1, .0001))
km = KMeans(init='random', n_clusters=num_cluster, verbose=1, n_init=1, max_iter=1, random_state=seed)
km.fit(sp.array(list(zip(x, y))))
Z = km.predict(sp.c_[mx.ravel(), my.ravel()]).reshape(mx.shape)
plot_clusters(x, y, "Clustering Iteration 1", km=km)
...
# 2 iterations ####################
km = KMeans(init='random', n_clusters=num_cluster, verbose=1,
            n_init=1, max_iter=2,
            random_state=seed)
km.fit(sp.array(list(zip(x, y))))
...
plot_clusters(x, y, "Clustering iteration 2", km=km)
pylab.imshow(Z, interpolation='nearest',
             extent=(mx.min(), mx.max(), my.min(), my.max()),
             cmap=pylab.cm.Blues,
             aspect='auto', origin='lower')
...
plot_clusters(x, y, "Clustering iteration 10", km=km)
pylab.imshow(Z, interpolation='nearest',
             extent=(mx.min(), mx.max(), my.min(), my.max()),
             cmap=pylab.cm.Blues,
             aspect='auto', origin='lower')
